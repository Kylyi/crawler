...
export function highlight<T = IItem>(
  ...
  const { keys = [], highlightClassName = 'fuse-highlighted', displayKeys, itemGetter } = options || {}
  ...
  const highlightedResult = fuseSearchResult
    ...
    .map(({ item, matches, score }) => {
      ...
      keys.forEach((key, idx) => {
        ...
        highlighted = match
          ? generateHighlightedText(highlighted, match.value, match.indices, displayText)
