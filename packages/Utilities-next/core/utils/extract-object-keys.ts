...
export function extractObjectKeys(
  ...
  if (Array.isArray(obj)) {
    ...
    return obj.length > 0
      ? extractObjectKeys(obj[0], { prefix: prefix ? `${prefix}.[n]` : '[n]', keepObjectKeys, omitKeys })
      : []
  }
  ...
  if (obj && typeof obj === 'object') {
    ...
    // Process nested keys
    const nestedKeys = Object.entries(obj as IItem).flatMap(([key, value]) => {
      return extractObjectKeys(value, { prefix: prefix ? `${prefix}.${key}` : key, keepObjectKeys, omitKeys })
    })
