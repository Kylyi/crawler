...
import type { IZodValidationOutput } from '../types/zod/zod-validation-output'
import { get, merge, set } from 'lodash-es'
import { computed, getCurrentInstance, inject, onUnmounted, provide, ref, toValue, watch } from 'vue'
import type { Ref } from 'vue'
import { toReactive, useThrottleFn, watchPausable } from '@vueuse/core'

type IZodInstanceValidationItem = {
  id: string
  componentName?: string
  errors: IZodValidationItem[]
  validateFn: (validateNested?: boolean, resumeWatch?: boolean) => Promise<boolean>
 ...
 */
function checkHasValidation<T extends ZodSchemaObject>(schemasOrOptions?: T | IZodValidationOptions) {
...
export function useZodOld<T extends ZodSchemaObject>(options?: IZodValidationOptions): IZodValidationOutput<T>
...
export function useZodOld<T extends ZodSchemaObject>(
  ...
  const requiredFields = inject(`${scopedInjectionName}_requiredFields`, ref({}) as Ref<IItem>)
  const validations = inject(scopedInjectionName, ref([])) as Ref<IZodInstanceValidationItem[]>
  ...
  const nestedValidations = computed(() => {
    const selfIdx = validations.value.findIndex((validation) => validation.componentName === componentName)
  ...
  const allErrorsByCleanPath = computed(() => {
    return Object.entries(allErrorsByPath.value).reduce(
      (agg, [key, value]) => {
  ...
  async function validate(validateNested = true, resumeWatch = true) {
    if (validateNested) {
      await Promise.all(nestedValidations.value.slice(1).map((validation) => validation.validateFn(false, resumeWatch)))
    }
    ...
    const existingValidationIdx = validations.value.findIndex((validation) => validation.id === instanceId)

    // When validations of this instance already exist, we replace them
    if (existingValidationIdx !== -1) {
      const next = [...validations.value]
      next.splice(existingValidationIdx, 1, {
        id: instanceId,
        componentName,
  ...
  function createEmptyErrorStructureFromShape(shape: ZodSchemaObject, path: string): any {
  ...
  function createEmptyErrorStructure() {
    ...
    for (const key in schemas) {
      if (key in schemas) {
        ...
        if (schemaValue instanceof z.ZodObject) {
          const isRequired = !schemaValue.isOptional()
          const emptyStructure = createEmptyErrorStructureFromShape(schemaValue.shape as ZodSchemaObject, key)
  ...
  function reset(shouldPause = true, resetNested = true) {
    ...
    if (selfValidationIdx > -1) {
      ...
      const next = [...validations.value]
      next.splice(selfValidationIdx, 1, {
        id: instanceId,
        componentName,
        errors: [],
        validateFn: validate,
    ...
    if (resetNested) {
      nestedValidations.value.forEach((validation) => validation.resetFn(shouldPause, false))
    }
  }
  ...
  const { pause, resume, isActive } = watchPausable(dataReactive, () => debouncedValidate(), { deep, immediate })
  ...
  // Lifecycle
  onUnmounted(() => {
    validations.value = validations.value.filter((validation) => validation.id !== instanceId)
  })
