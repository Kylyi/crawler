...
export async function runZakazkyGovDetailCrawl(
  ...
  const queue = await store.getTendersNeedingDetail(SOURCE_SLUG, limit)
  ...
  try {
    for (const [index, tender] of queue.entries()) {
      try {
        ...
        await store.enrichTender(SOURCE_SLUG, tender.externalId, patch)

        if (patch.documents?.length) {
          await store.replaceTenderDocuments(tender.id, patch.documents)
        }

        await store.markDetailFetched(SOURCE_SLUG, tender.externalId)
        processed++
    ...
    log.finish(status === 'success' ? 'detail crawl succeeded' : 'detail crawl completed with errors', {
      runId,
      processed,
      skipped,
      failed,
