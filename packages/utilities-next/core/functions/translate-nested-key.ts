...
export function translateNestedKey(
  ...
  const t = getUtilitiesConfig().translate ?? ((k: string) => k)

  const { nestedKey = 'self', plural = 1, translateOptions, prefix } = options ?? {}
  ...
  if (prefix) {
    ...
    for (const p of prefixes) {
      let prefixedKey = `${p}.${key}`
      const translatedPrefixed = t(prefixedKey, +plural, translateOptions)

      if (prefixedKey !== translatedPrefixed) {
        return translatedPrefixed
      }

      prefixedKey = `${p}.${key}.${nestedKey}`
